...
use crate::handlers::proxy::websocket::session::{
    CLOSE_TRY_AGAIN, REALTIME_WEBSOCKET_SESSION_LIMITS, WEBSOCKET_LOG_TRANSPORT,
};
use crate::handlers::proxy::websocket::transport::{
    client_message_to_upstream, close_client_socket, close_upstream_socket,
    connect_upstream_websocket, upstream_message_to_client, websocket_relay_frame_queue,
...
use super::protocol::{model_from_query, RealtimeUsageObserver};
...
struct RealtimeConnectionAdmission {
...
impl RealtimeConnectionAdmission {
    async fn acquire(
        state: &AppState,
...
pub(super) struct PreparedRealtimeWebSocket {
    ...
    _admission: RealtimeConnectionAdmission,
...
pub(super) async fn prepare_realtime_websocket(
    ...
    if !realtime_rpm_is_allowed(state, context).await? {
        return Err(rejection(
    ...
    let admission = RealtimeConnectionAdmission::acquire(
        state,
        &candidate.admission_plan,
    ...
    let upstream = match connect_upstream_websocket(
        &candidate.execution,
...
async fn realtime_rpm_is_allowed(
    state: &AppState,
    context: &WebSocketRequestContext,
) -> Result<bool, RealtimeWebSocketPreflightRejection> {
    match state.admin_security_ip_whitelisted(context.client_ip).await {
        Ok(true) => return Ok(true),
        Ok(false) => {}
        Err(error) => warn!(
            target: REALTIME_LOG_TARGET,
            event_name = "openai_realtime_ip_whitelist_check_failed",
            log_type = "ops",
            trace_id = %context.trace_id,
            client_ip = %context.client_ip,
            error = ?error,
            "gateway continued with Realtime RPM enforcement after IP whitelist check error"
        ),
    }
    let outcome = state
        .frontdoor_user_rpm()
        .check_and_consume(state, Some(&context.decision))
        .await
        .map_err(|_| {
            rejection(
                StatusCode::INTERNAL_SERVER_ERROR,
                "Realtime rate-limit admission failed",
            )
        })?;
    Ok(!matches!(outcome, FrontdoorUserRpmOutcome::Rejected(_)))
}
    ...
    let termination = {
        ...
        let client_to_upstream = {
            ...
            async move {
                ...
                tokio::select! {
                    reader_exit = &mut reader => writer.await.unwrap_or(reader_exit),
                    writer_exit = &mut writer => match writer_exit {
        ...
        let termination = tokio::select! {
            termination = &mut client_to_upstream => termination,
            termination = &mut upstream_to_client => termination,
            _ = &mut connection_deadline => "connection_duration_limit",
...
fn realtime_terminal_from_relay(
    ...
    let (disposition, status_code) = match termination {
        ...
        "connection_admission_lost" => (RealtimeSessionDisposition::Failed, 503),
