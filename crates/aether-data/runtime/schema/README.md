...
owned by the Postgres and SQLite adapters under `../../adapters/`.
...
- `generate` renders `logical/*.toml` through `aether-data-schema` into
  `generated/{postgres,sqlite}`. This is a build output, not another SQL
  source to maintain.
...
| `drivers/{postgres,sqlite}/` | Current maintenance fragments for executable SQL. | Edit only for deployment compatibility, ordering, or generator gaps. |
...
| Path | Role | Edit policy |
|---|---|---|
| `generated/{postgres,sqlite}/` | Machine-written SQL emitted from `logical/*.toml` for audit and drift detection. | Do not edit; regenerate with `compose_schema.sh generate`. |
| `../../adapters/{postgres,sqlite}/migrations/` | Runtime SQL artifacts embedded by each database adapter. | Regenerate through `compose_schema.sh compose`; do not edit independently. |
...
clean baseline table set and the portable SQLite table-creation
...
The existing `drivers/postgres` and `drivers/sqlite`
...
SQLite uses numbered domain fragments (`001_identity.sql` through
`006_usage.sql`) because its baseline is shorter and already organized by
domain.

The schema migration tests compose these manifests too, so fragment drift is
