INSERT INTO "usage" (
  ...
  COALESCE(
    CASE
      WHEN ($52::json->>'upstream_is_stream') IN ('true', 'false')
      THEN ($52::json->>'upstream_is_stream')::boolean
      ELSE NULL
    END,
    COALESCE($20, FALSE)
  ),
  ...
  COALESCE(
    ...
    COALESCE($21, 0)
      + COALESCE($22, 0)
      + COALESCE(
        NULLIF(COALESCE($24, 0), 0),
        COALESCE($25, 0) + COALESCE($26, 0),
        0
      )
      + COALESCE($27, 0)
  ),
  ...
  $52::json,
  CASE
    WHEN $53 IS NULL THEN NULL
    ELSE TO_TIMESTAMP($53::double precision)
  END,
  COALESCE(TO_TIMESTAMP($54::double precision), NOW()),
  COALESCE(
    NULLIF($55::bigint, 0),
    CAST(EXTRACT(EPOCH FROM COALESCE(TO_TIMESTAMP($54::double precision), NOW())) AS BIGINT)
  )
)
...
DO UPDATE SET
  ...
  provider_name = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.provider_name, "usage".provider_name) ELSE "usage".provider_name END,
  model = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.model, "usage".model) ELSE "usage".model END,
  target_model = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN CASE WHEN EXCLUDED.status IN ('completed', 'failed', 'cancelled') THEN EXCLUDED.target_model ELSE COALESCE(EXCLUDED.target_model, "usage".target_model) END ELSE "usage".target_model END,
  provider_id = CASE WHEN ("usage".billing_status = 'pending' AND $60) OR ("usage".billing_status <> 'pending' AND "usage".provider_id IS NULL AND ("usage".provider_endpoint_id IS NULL OR "usage".provider_endpoint_id = EXCLUDED.provider_endpoint_id) AND ("usage".provider_api_key_id IS NULL OR "usage".provider_api_key_id = EXCLUDED.provider_api_key_id)) THEN COALESCE(EXCLUDED.provider_id, "usage".provider_id) ELSE "usage".provider_id END,
  provider_endpoint_id = CASE WHEN ("usage".billing_status = 'pending' AND $60) OR ("usage".billing_status <> 'pending' AND "usage".provider_endpoint_id IS NULL AND ("usage".provider_id IS NULL OR "usage".provider_id = EXCLUDED.provider_id) AND ("usage".provider_api_key_id IS NULL OR "usage".provider_api_key_id = EXCLUDED.provider_api_key_id)) THEN COALESCE(EXCLUDED.provider_endpoint_id, "usage".provider_endpoint_id) ELSE "usage".provider_endpoint_id END,
  provider_api_key_id = CASE WHEN ("usage".billing_status = 'pending' AND $60) OR ("usage".billing_status <> 'pending' AND "usage".provider_api_key_id IS NULL AND ("usage".provider_id IS NULL OR "usage".provider_id = EXCLUDED.provider_id) AND ("usage".provider_endpoint_id IS NULL OR "usage".provider_endpoint_id = EXCLUDED.provider_endpoint_id)) THEN COALESCE(EXCLUDED.provider_api_key_id, "usage".provider_api_key_id) ELSE "usage".provider_api_key_id END,
  request_type = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.request_type, "usage".request_type) ELSE "usage".request_type END,
  api_format = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.api_format, "usage".api_format) ELSE "usage".api_format END,
  api_family = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.api_family, "usage".api_family) ELSE "usage".api_family END,
  endpoint_kind = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.endpoint_kind, "usage".endpoint_kind) ELSE "usage".endpoint_kind END,
  endpoint_api_format = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.endpoint_api_format, "usage".endpoint_api_format) ELSE "usage".endpoint_api_format END,
  provider_api_family = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.provider_api_family, "usage".provider_api_family) ELSE "usage".provider_api_family END,
  provider_endpoint_kind = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.provider_endpoint_kind, "usage".provider_endpoint_kind) ELSE "usage".provider_endpoint_kind END,
  is_stream = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.is_stream, "usage".is_stream) ELSE "usage".is_stream END,
  upstream_is_stream = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.upstream_is_stream, "usage".upstream_is_stream, "usage".is_stream, false) ELSE "usage".upstream_is_stream END,
  ...
  request_body = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN CASE
  ...
  request_body_compressed = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN CASE
  ...
  provider_request_body = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN CASE
  ...
  provider_request_body_compressed = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN CASE
  ...
  response_body = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN CASE
  ...
  response_body_compressed = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN CASE
  ...
  client_response_body = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN CASE
  ...
  client_response_body_compressed = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN CASE
  ...
  request_metadata = CASE WHEN "usage".billing_status = 'pending' AND $60 THEN COALESCE(EXCLUDED.request_metadata, "usage".request_metadata) ELSE "usage".request_metadata END,
