...
Aether Lite supports OpenAI compatible embedding requests through `POST /v1/embeddings`. Embedding requests are separate from chat and responses requests. They use `input`, never `messages`, and they are always non streaming.

## Quick Start

Run this against your Aether Lite gateway URL with a user API key that can access the model and the `openai:embedding` API format.
...
| `parameters` | Provider-specific embedding parameters. |
| `user` | Passed to OpenAI compatible providers. |
| `task` | Passed to OpenAI compatible embedding requests. |
...
Use string or string array input when routing to Gemini embedding providers. Token arrays are accepted by the OpenAI compatible public endpoint, while Gemini embedding requests require text input.
...
2. The provider model or mapping must expose `openai:embedding` or `gemini:embedding`.
