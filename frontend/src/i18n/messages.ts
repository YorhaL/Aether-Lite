export const messages = {
  'zh-CN': {
    ...
    'site.home.feature.cards.quota': '额度管理',
    'site.home.feature.cards.quotaDesc': '面向内部 API 分发的基础额度分配、查询与请求扣减',
  ...
  'en-US': {
    ...
    'site.home.feature.cards.quota': 'Quota management',
    'site.home.feature.cards.quotaDesc': 'Basic quota allocation, lookup, and request deductions for internal API distribution',
...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '模型、供应商和端点会在多个用户组之间叠加授权；unrestricted 仍表示不限制，deny_all 只是不授予额外权限。速率限制取更高额度，0 表示不限速；用户/API Key 自身限制仍会收窄最终权限。': 'Models, providers, and endpoints accumulate across multiple user groups. unrestricted still means no restriction, while deny_all grants no extra permission. Rate limits use the higher quota, and 0 means unlimited. User/API key limits still narrow the final permissions.',
  ...
  '额度限制 (美元/日)': 'Usage limit (USD/day)',
  '日限额': 'Daily limit',
  '日限额:': 'Daily:',
  '留空不增加 Key 级限制': 'Leave empty to add no key-level limit',
  '0 = 明确不限制': '0 = explicitly unlimited',
  '无日上限': 'No daily limit',
  '正数会进一步收窄用户额度限制；0 或留空不增加 Key 级限制。': 'A positive value further narrows the user usage limit. Zero or empty adds no key-level limit.',
  '正数会进一步收窄账户额度限制；0 或留空不增加 Key 级限制。': 'A positive value further narrows the account usage limit. Zero or empty adds no key-level limit.',
  '0 表示默认不限制；未单独配置的用户和独立 Key 会跟随这里': 'Zero means unlimited by default; users and standalone keys without an individual setting inherit this value.',
  ...
  'Aether Lite 提供高可插入的模块化管理机制，帮助连接外部服务和授权系统。': 'Aether Lite provides a highly pluggable module management system for connecting external services and authorization systems.',
  ...
  '选择此密钥支持的 API 格式及对应认证方式。': 'Select the API formats and authentication methods supported by this key.',
  ...
  '了解 Aether Lite 内部的日志记录、智能调度模式以及服务限制策略。': 'Learn about request logs, intelligent scheduling modes, and service limit policies inside Aether Lite.',
  ...
  '平台内置多个定时任务，用于模型列表同步、缓存清理及统计数据聚合等周期性操作。': 'The platform includes scheduled tasks for model-list sync, cache cleanup, statistics aggregation, and other recurring operations.',
  '文档核心': 'Documentation core',
  '深入理解 Aether Lite 运行时的核心模块和层级结构。': 'Understand the core modules and hierarchy of the Aether Lite runtime.',
  ...
  '建议首先检查【系统设置】中的请求体记录等级是否设为 `Full`，以便抓取完整的上下行数据。然后检查对应的【提供商端点】的 Base URL 和 API 格式是否正确匹配。': 'First check whether request-body logging in System settings is set to `Full` so complete upstream and downstream data can be captured. Then verify the provider endpoint Base URL and API format.',
...
const legacyPhraseEnglishMessages: Array<[string, string]> = [
  ...
  ['Aether Lite 的系统架构、请求处理流程和数据流向。', 'Aether Lite system architecture, request handling flow, and data flow.'],
  ['Aether Lite 在接收请求后，会按原始 API 格式统一管理路由。', 'Aether Lite manages routing while preserving the original API format.'],
  ['在内部完成自定义提供商和模型映射管理。客户端只需知道统一的模型名，Aether Lite 会根据映射规则找到对应的上游模型名称。', 'Aether Lite manages custom providers and model mappings. Clients only need the unified model name; Aether Lite resolves the corresponding upstream model name.'],
...
const legacyFallbackTokens: Array<[string, string]> = [
  ...
  ['默认额度限制（美元/日）', 'Default usage limit (USD/day)'],
  ['额度限制', 'Usage limit'],
