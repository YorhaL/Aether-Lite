...
describe('user management layout', () => {
  it('separates statistics and traffic control policy on desktop', () => {
    ...
    expect(policyCell).toContain('row.rateLimitSource')
    expect(policyCell).toContain('row.dailyUsageLimitSource')
    expect(policyCell).toContain('row.concurrentLimitSource')
  })
  ...
  it('shows policy in its own mobile section instead of status badges', () => {
    ...
    expect(cardSource).toContain('row.rateLimitSource')
    expect(cardSource).toContain('row.dailyUsageLimitSource')
    expect(cardSource).toContain('row.concurrentLimitSource')
